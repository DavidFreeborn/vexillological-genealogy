...
html = re.sub(r'<link\s+rel="stylesheet"\s+href="style.css"\s*/?>', lambda _: '<style>' + (dist / 'style.css').read_text() + '</style>', html)
for name in ['elk.bundled.js', 'layout.js', 'app.js']:
    code = (dist / name).read_text()
    if name == 'app.js':
        code = re.sub(r'''fetch\(["']data.json["']\)''', 'Promise.resolve({ok:true,json:async()=>ATLAS_DATA})', code)
        code = ('const OFFLINE_LAYOUTS=' + (dist / 'layouts.json').read_text() + ';\n'
                + 'const OFFLINE_WORKER_SOURCE=' + json.dumps((dist / 'elk-worker.min.js').read_text()) + ';\n'
